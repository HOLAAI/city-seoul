# -*- coding: utf-8 -*- 
# 2007~2016 연도별 대기오염 vs 사망자수 비교 그래프
import numpy as np
import math  as mt
...
def analyzeProc():
    ...
    # 연도별 사망자 csv 파일 읽어오기
    deadFileReader = ExcelFileReader('csv/DeadData.csv', ',')
    deadFileData = deadFileReader.csvRead()
    
    bf_date = ''
    yearIdx = 0
    monthIdx = 0
    deadData = [[0 for col in range(0)] for row in range(10)] 
    for data in deadFileData:
        for idx1, value in enumerate(data):
            if idx1 == 0 :
                if eq(bf_date, '') == True :
                    bf_date = value[:4]
    ...
    # 사망자 정규화 처리
    deadData = dataNormalUtil.twoNormalization(deadData)
